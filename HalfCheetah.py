...
from src.halfcheetah_utils import CnfndWrapper, rollout, noisy_rollout, T, dynamics

from src.models import Model
from src.learners.bc import BC
from learners.ccil_halfcheetah import CCIL,dynamics_wrapper
...
env = CnfndWrapper(gym.make("HalfCheetah-v2"))
...
expert_net = SAC.load("./src/experts/halfcheetah_expert")
def expert(s):
...
for _ in range(100):
    s_traj, _, J, p, v = rollout(expert, env, full_state=True)
    print(J)
...
with open("data/{0}_train_data/denoised_test_{0}_{1}.pkl".format(e, data_version), "wb") as f:
     data = {"s": denoised_test_s,"a": denoised_test_a,"P": P,"V": V}
     pickle.dump(data,f) 
...
V = []
for _ in range(100):
    s_traj, _, J, p, v= noisy_rollout(expert, env, sigma=2, full_state=True, n = 500, distribution= 'normal', hop = 3, TCN = TCN)
    print(J)
...
with open("data/{0}_train_data/noisy_test_{0}_{1}.pkl".format(e, data_version), "wb") as f:
     data = {"s": noisy_test_s,"a": noisy_test_a,"P": P,"V": V}
     pickle.dump(data,f)
...
for i in range(0, 5):
    for size in [10, 20, 30, 40, 50]:
        ...
        for j in range(size):
            print(j)
            s_traj, a_traj, _, p, v= noisy_rollout(expert, env, sigma=2, full_state=True,n = 500, distribution = 'normal', hop = 3, TCN = TCN)
        ...
        with open("data/{0}_train_data/train_{1}_{2}_{0}_{3}.pkl".format(e, size, i, data_version), "wb") as f:
            data = {"s": s_trajs,"a": a_trajs,"P": P,"V": V}
            pickle.dump(data,f) 
...
def eval_policy(pi, env, noisy=False):
    ...
    for _ in range(100):
        if noisy:
            s_traj, a_traj, J = noisy_rollout(pi, env, sigma = 2, n = 500, distribution= 'nromal', hop = 3, TCN = TCN)
        else:
...
if __name__ == '__main__':
    ...
    for i in range(0, 5):
        ...
        for size in [10, 20, 30, 40, 50]:
            ...
            s_trajs,a_trajs,p_trajs,v_trajs = [],[],[],[]
            ...
            p_trajs = all_traj["P"]
            v_trajs = all_traj["V"]
            expert_trajs = [[s_trajs[i], a_trajs[i], p_trajs[i], v_trajs[i]] for i in range(len(s_trajs))]
            ...
            pi_BC = BC(expert_trajs, Model(env.observation_space.shape[0], env.action_space.shape[0]).cuda(), wd=5e-4, device = device)
            ...
            pi_CCIL = CCIL(expert_trajs,
                            Model(env.observation_space.shape[0], env.action_space.shape[0]).cuda(),
                            dynamics_wrapper, pi_BC=pi_BC, wd=4e-6,env = env,device=device, workers = size,hop = hop)
