...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(self):
        ...
        self.u_past_7 = np.random.normal() * self.sigma
    ...
    def step(self, action,index=0,step = 1000, hop = 3, distribution = 'normal'):
        self.t += 1
        if self.confounding:
            if index == -1:
                ...
                self.u_past = u
            if index >0 and self.t in range(index,index + step): 
                if distribution == 'normal':
                    u = np.random.normal() * self.sigma
                elif distribution == 'uniform':
                    u = np.random.uniform() * self.sigma
                elif distribution == 'exponential':
                    u = np.random.exponential(1.5) * self.sigma
                elif distribution == 'gamma':
                    u = np.random.gamma(1,2) * self.sigma
                ...
                if hop == 3:
                    action = action * u * self.u_past_1 * self.u_past_2 * 0.1
                    self.u_past_1 , self.u_past_2 = u,self.u_past_1
                elif hop == 4:
                    action = action * u * self.u_past_1 * self.u_past_2 * self.u_past_3 * 0.1
                    self.u_past_1 , self.u_past_2, self.u_past_3 = u,self.u_past_1,self.u_past_2
                elif hop == 5:
                    action = action * u * self.u_past_1 * self.u_past_2 * self.u_past_3 * self.u_past_4 * 0.1
                    self.u_past_1 , self.u_past_2, self.u_past_3, self.u_past_4 = u,self.u_past_1,self.u_past_2,self.u_past_3
                elif hop == 6:
                    action = action * u * self.u_past_1 * self.u_past_2 * self.u_past_3 * self.u_past_4 * self.u_past_5 * 0.1
                    ...
                    action = action * u * self.u_past_1 * self.u_past_2 * self.u_past_3 * self.u_past_4 * self.u_past_5 * self.u_past_6 * 0.1
